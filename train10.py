...
down_sample_params = [1]
lidar = {i:[] for i in down_sample_params}
servo = {i:[] for i in down_sample_params}
speed = {i:[] for i in down_sample_params}
test_lidar = {i:[] for i in down_sample_params}
test_servo = {i:[] for i in down_sample_params}
test_speed = {i:[] for i in down_sample_params}
model_name = 'TLN'
model_files = [
...
dataset_path = [
    ...
    './Dataset/f2.db3',
    './Dataset/f4.db3',
    # './Dataset/Forza_GLC_smile_PP_edgecases_0.db3',
    # './Dataset/jfr1.db3',
    # './Dataset/jfr2.db3',
    # './Dataset/jfrv5_opp.db3',
    # './Dataset/jfrv6_opp.db3'
]
...
for param in down_sample_params:
    for pth in dataset_path:
        storage_opts = StorageOptions(uri=pth, storage_id='sqlite3')
        ...
        while reader.has_next():
            ...
            if topic == 'Lidar' or topic == 'scan':
                msg = deserialize_message(serialized_msg, LaserScan)
                cleaned = msg.ranges[:1080][::param]
                lidar_data.append(cleaned)

            elif topic == 'Ackermann' or topic == 'drive':
        ...
        lidar[param].extend(x_train_bag)
        servo[param].extend(y_train_bag[:, 0])
        speed[param].extend(y_train_bag[:, 1])

        test_lidar[param].extend(x_test_bag)
        test_servo[param].extend(y_test_bag[:, 0])
        test_speed[param].extend(y_test_bag[:, 1])
...
for param in down_sample_params:
    lidar[param] = np.asarray(lidar[param])
    lidar[param] = np.expand_dims(lidar[param], axis=-1)
    servo[param] = np.asarray(servo[param])
    speed[param] = np.asarray(speed[param])
    speed[param] = linear_map(speed[param], min_speed, max_speed, 0, 1)
    test_lidar[param] = np.asarray(test_lidar[param])
    test_lidar[param] = np.expand_dims(test_lidar[param], axis=-1)
    test_servo[param] = np.asarray(test_servo[param])
    test_speed[param] = np.asarray(test_speed[param])
    test_speed[param] = linear_map(test_speed[param], min_speed, max_speed, 0, 1)

datasets = []
for param in down_sample_params:
    data = tf.data.Dataset.from_tensor_slices((lidar[param],np.stack([servo[param],speed[param]],axis=1)))
    data = data.batch(batch_size)
    datasets.append(data)
test_datasets = []
for param in down_sample_params:
    data = tf.data.Dataset.from_tensor_slices((test_lidar[param],np.stack([test_servo[param],test_speed[param]],axis=1)))
    data = data.batch(batch_size)
    test_datasets.append(data)
train_dataset = datasets[0]
for data in datasets[1:]:
    train_dataset = train_dataset.concatenate(data)
test_dataset = test_datasets[0]
for data in test_datasets[1:]:
    test_dataset = test_dataset.concatenate(data)
...
num_lidar_range_values = len(lidar[down_sample_params[0]][0])
...
print(f"{model_name}.keras is saved.")
